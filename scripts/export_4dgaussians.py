...
def main():
    ...
    p.add_argument('--train-views', type=int, nargs='+', required=True)
    p.add_argument('--test-views', type=int, nargs='+', required=True)
    p.add_argument('--scale', type=float, default=.5)
    args = p.parse_args()
    if not 0 < args.scale <= 1 or set(args.train_views) & set(args.test_views):
        p.error('Scale must be in (0,1]; training and test cameras must be disjoint.')
    ...
    selected = sorted(set(args.train_views + args.test_views))
    if set(selected) - by_id.keys():
        p.error('Unknown camera IDs')
    ...
    points = vertices[rng.choice(len(vertices), min(40000,len(vertices)), replace=False)]
